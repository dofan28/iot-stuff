...
void setupLittleFS()
{
  if (LittleFS.begin())
  {
    Serial.println(F("LittleFS berhasil dimuat"));
  }
  else
  {
    Serial.println(F("Gagal memuat LittleFS"));
    LittleFS.format();
    if (LittleFS.begin())
    {
      Serial.println(F("LittleFS berhasil diformat dan dimuat"));
    }
    else
    {
      Serial.println(F("Gagal memformat LittleFS"));
    }
  }
}
...
void handleSaveSettings()
{
  if (server.hasArg("api_url"))
    apiConfig.baseUrl = server.arg("api_url");
  if (server.hasArg("terminal_id"))
    apiConfig.terminalId = server.arg("terminal_id");
  if (server.hasArg("api_token"))
    apiConfig.apiToken = server.arg("api_token");
  if (server.hasArg("payment_amount"))
    state.paymentAmount = server.arg("payment_amount").toFloat();
  ...
  if (httpCode == 200)
  {
    ...
    if (!error && respDoc["success"].as<bool>())
    {
      ...
      if (data.containsKey("allowed_user_types"))
      {
        ...
        for (size_t i = 0; i < allowed_types.size() && i < 3; i++)
        {
